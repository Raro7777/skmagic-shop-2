/**
 * SK매직 26년 6월 타사보상 가격 적용 (신규, 단품 only).
 *
 * 데이터 원본: 26년 6월 별첨 "타사보상 가격" 2페이지 표
 *
 * 5월 대비 변경:
 *   - WPUIAC506 (MEGA ICE)  7년 방문 가격 43,500 → 42,500
 *   - WPUIAC606 (MEGA ICE mini) — 신규 prefix 추가
 *   - 나머지 항목 5월과 동일
 *
 * 적용 방법: Product.priceMatrix JSON 의 각 옵션에
 *   - rivalCompensationPrice
 *   - rivalCompensationHalfPriceMonths
 *  업데이트. 카드할인은 별개로 적용.
 */
...
// 26.6월 별첨 타사보상 가격 (3년=36월, 5년=60월, 6년=72월, 7년=84월)
const RIVAL_BY_PREFIX: Record<string, Group> = {
  ...
  WPUIAC506: { // MEGA ICE — 6월: 84개월 방문 가격만 43,500 → 42,500
    방문: [{ months: 36, price: 48500 }, { months: 60, price: 46000 }, { months: 72, price: 45000 }, { months: 84, price: 42500 }],
    셀프: [{ months: 36, price: 46500 }, { months: 60, price: 44000 }, { months: 72, price: 43000 }, { months: 84, price: 41500 }],
  },
  WPUIAC606: { // 🆕 MEGA ICE mini (6월 신규)
    방문: [{ months: 36, price: 41900 }, { months: 60, price: 38900 }, { months: 72, price: 37900 }, { months: 84, price: 36900 }],
    셀프: [{ months: 36, price: 39900 }, { months: 60, price: 36900 }, { months: 72, price: 35900 }, { months: 84, price: 34900 }],
  },
  // ── 직수 ──────────────────────────────────────────────────────
  WPUMAC306: { // 투워터
  ...
  WPUJCC104: { // 초소형 다른 prefix — 동일 가격
    방문: [{ months: 36, price: 38000 }, { months: 60, price: 32000, halfPriceMonths: 3 }, { months: 72, price: 30500, halfPriceMonths: 3 }, { months: 84, price: 29000, halfPriceMonths: 3 }],
    셀프: [{ months: 36, price: 35000 }, { months: 60, price: 29000, halfPriceMonths: 3 }, { months: 72, price: 27500, halfPriceMonths: 3 }, { months: 84, price: 26000, halfPriceMonths: 3 }],
  },
  WPUJAC125: { // 초소형 라이트
    방문: [{ months: 36, price: 35000, halfPriceMonths: 3 }, { months: 60, price: 30000, halfPriceMonths: 3 }, { months: 72, price: 28500, halfPriceMonths: 3 }, { months: 84, price: 27000, halfPriceMonths: 3 }],
    셀프: [{ months: 36, price: 32000, halfPriceMonths: 3 }, { months: 60, price: 27000, halfPriceMonths: 3 }, { months: 72, price: 25500, halfPriceMonths: 3 }, { months: 84, price: 24000, halfPriceMonths: 3 }],
  },
  WPUPBC204: { // 뉴미니 — 5년/6년만
...
function findRow(group: Group, mode: "방문형" | "셀프형" | null, months: number): Row | undefined {
  const list = mode === "셀프형" ? group.셀프 : group.방문; // null = 방문 디폴트
...
async function main() {
  ...
  for (const p of products) {
    ...
    const next: PriceOpt[] = matrix.map(o => {
      ...
      if (!row) {
        // 6월 시점에 prefix 매칭 후 해당 옵션이 표에 없으면 기존 값 유지 (clear 하지 않음)
        return o;
      }
      localUpdates++;
      return {
  ...
  console.log(`📋 26.6월 타사보상 가격 적용 결과`);
  console.log(`  대상 prefix : ${Object.keys(RIVAL_BY_PREFIX).length}개 (WPUIAC606 신규 포함)`);
  console.log(`  적용된 Product : ${touchedProducts}개`);
  console.log(`  옵션 갱신 합계 : ${withRival}건 (전체 옵션 ${totalOptions}건)`);
  console.log(`\n샘플:`);
  for (const s of sampleHits.slice(0, 30)) {
    console.log(`  ${s.code.padEnd(15)}  ${s.name.padEnd(35)}  +${s.updates}건`);
  }
  if (sampleHits.length > 30) console.log(`  ... +${sampleHits.length - 30}개 더`);

  // MEGA ICE mini 적용 결과 검증
  const mini = await prisma.product.findFirst({
    where: { productCode: { startsWith: "WPUIAC606" } },
    select: { productCode: true, name: true, priceMatrix: true },
  });
  if (mini) {
    const opts = mini.priceMatrix as unknown as PriceOpt[] | null;
    if (opts) {
      console.log(`\n[검증 — MEGA ICE mini] ${mini.productCode} ${mini.name}`);
      for (const o of opts) {
        const rp = o.rivalCompensationPrice ?? "—";
        const half = o.rivalCompensationHalfPriceMonths ? ` (${o.rivalCompensationHalfPriceMonths}개월 반값)` : "";
        console.log(`  ${(o.mode ?? "단일").padEnd(6)} ${String(o.contractPeriod).padStart(2)}개월 → 일반 ${o.rentalPrice}원 / 타사 ${rp}${half}`);
      }
    }
  }

  // MEGA ICE 7년 방문 가격 변경 검증
  const mega = await prisma.product.findFirst({
    where: { productCode: { startsWith: "WPUIAC506" } },
    select: { productCode: true, name: true, priceMatrix: true },
  });
  if (mega) {
    const opts = mega.priceMatrix as unknown as PriceOpt[] | null;
    const visit84 = opts?.find(o => (o.mode === "방문형" || o.mode === null) && o.contractPeriod === 84);
    console.log(`\n[검증 — MEGA ICE 84개월 방문] ${mega.productCode}`);
    console.log(`  타사보상 = ${visit84?.rivalCompensationPrice ?? "—"} (목표 42500)`);
  }
}
